...
class JupiterX_Core_Control_Panel_Layout_Builder {
	...
	private function get_posts() {
		...
		// For non-supported type in Elementor free we use another key , because maintenance isn't supported by elementor.
		if ( true === $this->elementor_exceptions_type( $type ) ) {
			$args['meta_query'][0]['value'] = 'section';
		...
		foreach ( $posts as $key => $data ) {
			$data->editor_url  = Plugin::$instance->documents->get( $data->ID )->get_edit_url(); // elementor editor
	...
	private function create_post() {
		...
		// User extra meta key for types that isn't supported with elementor / elementor free.
		if ( true === $this->elementor_exceptions_type( $type ) ) {
			$args['meta_input']['_elementor_template_type'] = 'section';
		}
		...
		if ( 'sticky-header' === $type ) {
			$args['meta_input']['_elementor_template_type']      = 'header';
			$args['meta_input']['jx-layout-builder-hidden-type'] = 'sticky-header';
		}
		...
		if ( filter_has_var( INPUT_POST, 'sticky' ) ) {
			$response = [
				'id'    => $id,
				'url'   => $editor_url,
				'title' => $title,
	...
	private function manage_get_woo_sections( $type, $args ) {
		$args['meta_query'][0]['value']   = [ 'section', $type ];
		...
		if ( class_exists( 'Jet_Woo_Builder' ) && 'product' === $type ) {
			$args['meta_query'][0]['value']   = [ 'product', 'jet-woo-builder', 'section' ];
			$args['meta_query'][0]['compare'] = 'IN';
			return $args;
		}

		if ( class_exists( 'Jet_Woo_Builder' ) && 'product-archive' === $type ) {
			$args['meta_query'][0]['value']   = [ 'product-archive', 'jet-woo-builder-archive', 'section' ];
			$args['meta_query'][0]['compare'] = 'IN';
			return $args;
		}
	...
	private function export() {
		$post_id = filter_input( INPUT_POST, 'post', FILTER_SANITIZE_STRING );
	...
	private function import() {
		...
		if ( 'zip' === $ext ) {
			$extracted_files = Plugin::$instance->uploads_manager->extract_and_validate_zip( $json );
	...
	private function import_single_template( $file_path, $type ) {
		...
		if ( true === $is_section ) {
			update_post_meta( $id, 'jx-layout-type', $old_type );
		}
