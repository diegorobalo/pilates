...
def init_users_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS ip_location_cache (
...
def get_location_from_ip(ip_address):
    """
    Obtiene la ubicación de una IP usando caché y API de fallback.
    Retorna: {"city": str, "region": str, "country": str, "latitude": float, "longitude": float}
    """
    # Saltar IPs locales
    if ip_address.startswith(("10.", "192.168.", "127.", "172.")):
        return {"city": "Local", "region": "Local", "country": "Local", "latitude": 0, "longitude": 0}
    
    # Buscar en caché
    conn = sqlite3.connect(str(USERS_DB))
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("SELECT city, region, country, latitude, longitude FROM ip_location_cache WHERE ip_address=?", (ip_address,))
    cached = c.fetchone()
    ...
    # Obtener de API (ip-api.com es gratuito y no requiere clave)
    try:
        response = requests.get(f"http://ip-api.com/json/{ip_address}?fields=city,regionName,country,lat,lon", timeout=5)
        if response.status_code == 200:
            ...
            # Guardar en caché
            c.execute("""INSERT OR REPLACE INTO ip_location_cache 
                        (ip_address, city, region, country, latitude, longitude)
                        VALUES (?, ?, ?, ?, ?, ?)""",
                     (ip_address, location["city"], location["region"], location["country"],
...
<div class="field"><button type="submit" class="btn" style="margin-top:16px">Cambiar</button></div>
    ...
    def api_stats():
        ...
        # Obtener hora de la PRIMERA conexión del día ✅ MEJORA 1
        c.execute("""SELECT MIN(first_seen) as first_time FROM daily_uptime WHERE date=?""", (today,))
        first_time_row = c.fetchone()
        first_connection_time = None
        if first_time_row and first_time_row["first_time"]:
            try:
                # Parsear la hora en formato HH:MM:SS
                first_time_str = first_time_row["first_time"]
                if len(first_time_str) >= 8:
                    hour, minute = first_time_str[0:2], first_time_str[3:5]
                    hour_int = int(hour)
                    am_pm = "am" if hour_int < 12 else "pm"
                    hour_12 = hour_int if hour_int <= 12 else hour_int - 12
                    if hour_12 == 0: hour_12 = 12
                    first_connection_time = f"{hour_12}:{minute} {am_pm}"
            except:
                pass
    ...
    def api_set_user_sector():
        """Admin endpoint para asignar sector a usuario. ✅ MEJORA 2"""
    ...
    @app.route("/api/unassigned-users")
    @login_required
    def api_unassigned_users():
        """Retorna usuarios sin asignar a un sector. ✅ MEJORA 2"""
        db = get_db(); c = db.cursor()
        days = int(request.args.get("days", 30))
        since = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        
        # Obtener todos los usuarios que se conectaron en los últimos N días
        c.execute("""SELECT DISTINCT username FROM vpn_sessions 
                     WHERE connected_at >= ? ORDER BY username""", (since,))
        all_users = [r["username"] for r in c.fetchall()]
        
        # Filtrar solo los que no tienen sector asignado
        unassigned = []
        for user in all_users:
            sector = get_user_sector(user)
            if sector == "Sin asignar":
                # Obtener estadísticas del usuario
                c.execute("""SELECT COUNT(*) as sessions,
                    ROUND(SUM(CASE WHEN disconnected_at IS NOT NULL THEN
                        (julianday(disconnected_at)-julianday(connected_at))*24
                    ELSE (julianday('now','localtime')-julianday(connected_at))*24
                    END), 2) as total_hours,
        ...
        unassigned.sort(key=lambda x: x["sessions"], reverse=True)
        return jsonify({"count": len(unassigned), "users": unassigned})

    @app.route("/api/monthly-activity")
    @login_required
    def api_monthly_activity():
        """Retorna actividad mensual con estadísticas útiles (últimos 30 días). ✅ MEJORA 3"""
        db = get_db(); c = db.cursor()
        days = 30
        since = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        ...
        c.execute("""SELECT date, 
                     ...
                     FROM daily_uptime WHERE date >= ?
                     GROUP BY date ORDER BY date DESC""", (since,))
        daily_data = []
        for r in c.fetchall():
            daily_data.append({
                "date": r["date"],
                ...
                "total_gb": round(r["total_bytes"]/1073741824, 2)
            })
        ...
        c.execute("""SELECT 
                     ...
                     SUM(total_seconds)/3600.0 as total_hours,
                     SUM(bytes_in + bytes_out) as total_bytes
                     FROM daily_uptime WHERE date >= ?""", (since,))
        summary = c.fetchone()
        ...
        return jsonify({
            ...
            "summary": {
                "days_with_activity": summary["days_with_activity"] or 0,
                "unique_users": summary["unique_users"] or 0,
                "total_hours": round((summary["total_hours"] or 0), 2),
                "total_gb": round((summary["total_bytes"] or 0)/1073741824, 2)
            },
            "daily": daily_data
        })
...
if __name__ == "__main__":
    ...
    app = create_app(str(dashboard_html))
    print("🟢 VPN Monitor Web Server iniciado")
